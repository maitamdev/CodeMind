...
const formatReasoningDuration = (duration?: number) => {
...
export const Reasoning = memo(
  ({
    ...
    const setDuration = (value: number | undefined) => {
...
const defaultGetThinkingMessage = (isStreaming: boolean, duration?: number) => {
  if (isStreaming || duration === 0) {
    return <span className="animate-pulse">Đang suy luận...</span>
  }
  if (duration === undefined) {
    return <p>Đã suy luận trong vài giây</p>
  }
  return <p>Đã suy luận trong {formatReasoningDuration(duration)}</p>
}
